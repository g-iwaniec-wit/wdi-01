...
import { b as De } from './CZKBnRv4.js';
function Fe(r) {
	let e = 0,
		t = W(0),
		i;
...
class xe {
		...
		((this.#t = e),
			...
			(this.#e = re(() => {
				if (v) {
					...
					se();
					const n = s.data === ie;
					if (s.data.startsWith(P)) {
						const a = JSON.parse(s.data.slice(P.length));
						this.#E(a);
	...
	#u(e) {
		var t = b,
			i = U,
			f = G;
		(N(this.#e), D(this.#e), j(this.#e.ctx));
		try {
			return (fe.ensure(), e());
		} catch (s) {
			return (oe(s), null);
					...
					for (var h = 0; h < l.length; h++) {
						...
						if (!E.has(d)) {
							...
							for (const O of [e, document]) {
								...
								var q = m.get(d);
								q === void 0
									? (O.addEventListener(d, $, { passive: u }), m.set(d, 1))
									: m.set(d, q + 1);
							}
						}
					}
					...
					for (var l of E)
						for (const u of [e, document]) {
							var h = R.get(u),
								d = h.get(l);
							--d == 0
								? (u.removeEventListener(l, $), h.delete(l), h.size === 0 && R.delete(u))
								: h.set(l, d);
						}
...
export { Be as a, He as d, Ve as h, Me as m, Pe as s, je as u };
